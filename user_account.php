...
if (isset($_POST['Login'])) {
    ...
    // Lookup user by SchoolID
    $query_user = "SELECT * FROM voters WHERE SchoolID='$SchoolID'";
    $result_user = mysqli_query($conn, $query_user);

    if (mysqli_num_rows($result_user) == 0) {
        $_SESSION['login_error'] = "School ID not found.";
        redirect('index.php');
    }
    ...
    $_SESSION['login_username']  = $user['Username']; // this is email
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'jayzmariano25@gmail.com';     // Your Gmail
        $mail->Password   = 'leez oekt nnsm cbkk';          // App password
        $mail->SMTPSecure = 'tls';
        $mail->Port       = 587;

        $mail->setFrom('jayzmariano25@gmail.com', 'OEVS OTP');
        $mail->addAddress($user['Username']); // Send to student's email
